...
CREATE TABLE IF NOT EXISTS `utilisateur` (
  ...
  `c_postal` int unsigned NOT NULL,
...
BEGIN
INSERT INTO alimfavori VALUES(IdUser,AlimCode,YEAR(SYSDATE()));
END//
DELIMITER ;
...
BEGIN
INSERT INTO utilisateur(Identifiant,pwd,Nom,Prenom,Naissance,`CODE Postale`,Telephone,Ville,Adresse) VALUES(Id,SHA2(mdp,512),Nom,Prenom,STR_TO_DATE(DateOfBirth,"%d/%c/%Y"),CodePostale,Tel,City,Address);
END//
DELIMITER ;
...
CREATE TABLE IF NOT EXISTS `alimfavori` (
  ...
  CONSTRAINT `FK_User` FOREIGN KEY (`Identifiant_User`) REFERENCES `utilisateur` (`Id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Listage des données de la table site.alimfavori : ~0 rows (environ)
...
CREATE TABLE IF NOT EXISTS `score_sante` (
  ...
  CONSTRAINT `FK_User_score` FOREIGN KEY (`IdentifiantUser`) REFERENCES `utilisateur` (`Id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
